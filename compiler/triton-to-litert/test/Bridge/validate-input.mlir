// RUN: triton-to-litert-opt %s > %t.before
// RUN: triton-to-litert-opt --verify-triton-to-litert-bridge-input --verify-each %s > %t.after
// RUN: diff %t.before %t.after
// RUN: FileCheck %s --input-file=%t.after --check-prefix=SHAPE
// RUN: FileCheck %s --input-file=%t.after --check-prefix=MAKERS
// RUN: FileCheck %s --input-file=%t.after --check-prefix=LOADS
// RUN: FileCheck %s --input-file=%t.after --check-prefix=GENERICS
// RUN: FileCheck %s --input-file=%t.after --check-prefix=STORES
...
// SHAPE-LABEL: func.func @vector_add(
// SHAPE-NOT: memref
// SHAPE-NOT: bufferization
// SHAPE-NOT: "tfl.
// MAKERS-COUNT-3: tts.make_tptr
// MAKERS-NOT: tts.make_tptr
// LOADS-COUNT-2: "tts.load"
// LOADS-NOT: "tts.load"
